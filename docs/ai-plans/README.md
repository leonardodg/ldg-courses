...
| 2026-09-17 | [Assinatura SaaS: planos Start e PRO](2026-09-17-assinatura-saas-planos-start-e-pro.md) | `inacabado` | desenho comercial (2 planos, comissão e mensalidade por tier) e arquitetura (`paymentarea 'plan'`, conta da plataforma) — implementado nas Fases A–E, **falta a prova com dinheiro real** |
| 2026-09-17 | [Assinatura SaaS: implementação, Fases A–E](2026-09-17-assinatura-saas-implementacao-fases-a-e.md) | `inacabado` | planos no banco, `service_provider` com a paymentarea `'plan'`, os 3 gateways generalizados, botão de pagar em `company.php` — worktree `saas-planos-start-pro`; **falta a prova com dinheiro real**, roteiro em `docs/data-validation/assinatura-saas-plano-empresa.md` |
| 2026-09-17 | [Ativação de empresa parceira — desenho das telas](2026-09-17-ativacao-empresa-desenho-telas.md) | `pendente` | desenho de 11 telas no m3e-canvas (wizard, painel ativo, pagamento, plano, documento, termos, aprovação, bloco de curso) — **aguarda aprovação do layout** antes da implementação do `block_marketplace` |
| 2026-09-23 | [CS com `moodle-extra` e hub de produto](2026-09-23-cs-moodle-extra-e-hub-produto.md) | `inacabado` | **Etapa 0 executada e merged** (PR #124, merge `2570521ddad`; phpcs 0 issues / 740 testes; `.phpcs.xml` + 134 `public const` + CI `moodle-extra` + doc de CS). **Etapa 1 hub entregue** em `feature/produto-hub` (7 docs em `docs/produto/`, ADR-0014 supersede do 0005, gate de contradições) — **PR em aberto**; falta aceite do gate, números a confirmar e a fase de vídeo |
